...
$FoldersToRemove = Get-ChildItem .\ -include bin,obj,.vs,node_modules,TestResults -Recurse | where {$_ -notmatch '_tools' -and $_ -notmatch '_build'} | foreach {$_.fullname}
...
{			
	rmdir .vs -Force -Recurse -Confirm:$false;
